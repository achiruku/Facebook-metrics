...
# read the csv
data <- read.csv(file="/Users/achirukuri/Downloads/dataset_Facebook1.csv",header=T, as.is=TRUE)
# check na row
sum(is.na(data))
# clean na row
data=na.omit(data)
...
#Paid-Posts
paid.data<-read.csv(file="/Users/achirukuri/Downloads/Facebook_metrics/Paid.csv",header=TRUE)
paid.views<-c(0)
for(i in 1:180)
   paid.views[i]=0
for(i in 1:139)
...
#Unpaid-Posts
Unpaid.data<-read.csv(file="/Users/achirukuri/Downloads/Facebook_metrics/Unpaid.csv",header=TRUE)
Unpaid.views<-c(0)
for(i in 1:180)
   Unpaid.views[i]=0
for(i in 1:139)
